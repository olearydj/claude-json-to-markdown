...
import json
from typing import Optional # Not strictly needed for tests but good for consistency if used
...
def test_create_slug_special_chars_and_spaces():
    assert create_slug("  Test with !@#$ and   multiple spaces  ") == "test-with-and-multiple-spaces"
...
def test_create_slug_max_length_truncation():
    ...
    assert create_slug(long_string, max_length=20) == "this-is-a-very-long" # Expected: 20 chars
...
def test_load_and_validate_valid_json(mocker):
    ...
    mocker.patch("json.load", return_value=sample_data) # Ensure json.load is also consistently mocked
...
def test_load_and_validate_invalid_json(mocker, caplog):
    mocker.patch("pathlib.Path.open", mocker.mock_open(read_data='[{"name": "Test1"')) # Malformed JSON string, but valid Python string
...
def test_load_and_validate_json_not_a_list(mocker, caplog):
    ...
    assert result is None
    assert "Error: The JSON file's top-level structure is not a list of conversations." in caplog.text
...
def test_generate_filename_standard():
    ...
    assert generate_filename(conv_data, conv_name) == "2023-07-15_my-awesome-conversation_abc.md"
...
def test_generate_filename_uuid_no_hyphen():
    ...
    assert generate_filename(conv_data, conv_name) == "2023-07-15_no-hyphen-uuid_fulluuid.md"
...
def test_generate_markdown_content_basic():
    conv_data = {
        "uuid": "conv-uuid-001", "name": "Greeting Exchange", # name in conv_data for consistency, though conv_name param is used
        "created_at": "2023-01-01T10:00:00Z", "updated_at": "2023-01-01T11:00:00Z",
        "chat_messages": [
            {"sender": "User", "created_at": "2023-01-01T10:05:00Z", "text": "Hello there!"},
            {"sender": "AI", "created_at": "2023-01-01T10:06:00Z", "content": [{"text": "Hi User!"}]}
        ]
    }
...
def test_generate_markdown_content_no_messages():
    conv_data = {
        "uuid": "conv-uuid-002", "name": "Empty Talk",
...
def test_generate_markdown_content_with_attachments():
    conv_data = {
        "uuid": "conv-uuid-003", "name": "File Share",
        "created_at": "2023-01-03T00:00:00Z", "updated_at": "2023-01-03T00:00:00Z",
        "chat_messages": [
            {"sender": "User", "text": "See attached.", "created_at": "N/A",
...
def test_generate_markdown_content_sparse_message_data():
    conv_data = {
        "uuid": "conv-uuid-004", "name": "Sparse Message",
        "created_at": "N/A", "updated_at": "N/A",
        "chat_messages": [{"created_at": "A while ago"}] # No sender, no text, no content
    }
...
def test_write_markdown_file_io_error(mocker, caplog):
    ...
    success = write_markdown_file(filepath, content_lines, "Test Conv Fail", "test-uuid-fail")
...
def test_write_markdown_file_other_exception(mocker, caplog):
    ...
    success = write_markdown_file(filepath, content_lines, "Test Conv Unexp", "test-uuid-unexp")
